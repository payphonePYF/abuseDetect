...
def detect_abuse(sentences):
    ...
    for j,sentence in enumerate(sentences):
        ...
        # st.write(f"Query for sentence {j + 1}: {query}")
        sparkAPI.xinghuo(
            appid="870c51cc",
    ...
    results = {'句子{}'.format(i+1): '是骂人' if is_abusive(sparkAPI.latest_message_content[i]) else '不是骂人' for i, sentence in enumerate(sentences)}
    return results
abuse_num =0
not_abuse_num =0
